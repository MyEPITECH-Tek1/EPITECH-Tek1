...
def plot_data(vol, ph, second_derivatives):
    plt.figure(figsize=(10, 8))

    # Tracer la courbe de pH en fonction du volume
    plt.subplot(3, 1, 1)
    plt.plot(vol, ph, label='pH vs Volume', color='blue')
    plt.title('Titration Curve')
    plt.xlabel('Volume (ml)')
    plt.ylabel('pH')
    plt.grid(True)

    # Tracer la deuxième dérivée
    plt.subplot(3, 1, 3)
    volumes_second_deriv = [v for v, p in second_derivatives]
    ph_second_deriv = [p for v, p in second_derivatives]
    plt.plot(volumes_second_deriv, ph_second_deriv, label='Second Derivative', color='green')
    plt.title('Second Derivative of pH Curve')
    plt.xlabel('Volume (ml)')
...
def titration(file):
    ...
    plot_data(vol, ph, second_derivatives)